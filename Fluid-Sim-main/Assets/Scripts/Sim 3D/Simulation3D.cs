...
public class Simulation3D : MonoBehaviour
{
    ...
    public float boundsSizeCylinderHeight;
    public float boundsSizeCylinderRadius;
    public bool boundsShapeCube = true;
    public bool boundsShapeCylinder = false;
    public bool boundsShapeCone = false;
    ...
    public int whirldepth = 2;
    public float amplitude = 1.0f;
    public float sigma = 1.0f;
    public int numArms = 2;
    //test circle
    bool isKeyDownF1 = false;
    public Vector3 centerExternalForce;
    ...
    void Start()
    {
        ...
        getboundary = boundsSize;
        getRadius = boundsSizeCylinderRadius;
    ...
    void UpdateSettings(float deltaTime)
    {
        ...
        compute.SetFloat("boundsSizeCylinderHeight", boundsSizeCylinderHeight);
        compute.SetFloat("boundsSizeCylinderRadius", boundsSizeCylinderRadius);
        compute.SetBool("boundsShapeCube", boundsShapeCube);
        compute.SetBool("boundsShapeCylinder", boundsShapeCylinder);
        compute.SetBool("boundsShapeCone", boundsShapeCone);
        ...
        if (Input.GetKeyDown(KeyCode.F1))
        {
            ...
            isKeyDownF1 = !isKeyDownF1;
        }
        bool isPullInteraction = isKeyDownF1;

        float currInteractStrength = 0;
        if (isPullInteraction)
        {
            currInteractStrength = -interactionStrength;
        }

        compute.SetVector("interactionInputPoint", centerExternalForce);
        compute.SetFloat("interactionInputStrength", currInteractStrength);
        compute.SetFloat("interactionInputRadius", interactionRadius);
        compute.SetInt("whirldepth ", whirldepth);

        //compute.SetFloat("amplitude", amplitude);
        //compute.SetFloat("sigma ", sigma);
        //compute.SetInt("numArms", numArms);
        ...
        if (f2Click)
        {
            boundsSize.x = getboundary.x - getboundary.x * 0.2f * Mathf.Sin(1.8f * Time.time);
            boundsSizeCylinderRadius = getRadius - getRadius * 0.2f * Mathf.Sin(1.8f * Time.time);

            // Debug.Log($"{Time.time}   {boundsSize.x}");

        }
        else
        {
            boundsSize.x = getboundary.x;
            boundsSizeCylinderRadius = getRadius;
        }
        ...
        compute.SetBool("f2Click", f2Click);
        compute.SetBool("f3Click", f3Click);
